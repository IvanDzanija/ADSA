...
def from_points_help(
    result: List[Node], points: List[Point], start_index: int = 0
) -> List[Node]:
...
def query_priority_subtree(
    tree: List[Node], index: int, limit_value: float
) -> List[Point]:
    ...
    if node is not None and node is not INF_NODE and node.point.y >= limit_value:
...
def query(tree: List[Node], interval: Tuple[float, float], limit: float) -> List[Point]:
    ...
    while (
        node is not None
        and node is not INF_NODE
        and (interval[0] > node.med or interval[1] < node.med)
    ):
        point = node.point
        if node.point.y >= limit and interval[0] <= node.point.x <= interval[1]:
            result.append(point)

        if node.med < interval[0]:
            index *= 2
            index += 2
        else:
            index *= 2
    ...
    point = node.point
    if point.y >= limit and interval[0] <= point.x <= interval[1]:
        result.append(point)
    ...
    while node is not None and node is not INF_NODE and node.point.y >= limit:
        ...
        if node.point.y >= limit and interval[0] <= node.point.x <= interval[1]:
            result.append(point)
        if interval[0] <= node.med <= interval[1]:
            result += query_priority_subtree(tree, 2 * index + 2, limit)
            index *= 2
            index += 1
        else:
            index *= 2
    ...
    while node is not None and node is not INF_NODE and node.point.y >= limit:
        ...
        if node.point.y >= limit and interval[0] <= node.point.x <= interval[1]:
            result.append(point)
        if interval[0] <= node.med <= interval[1]:
            result += query_priority_subtree(tree, 2 * index + 1, limit)
            index *= 2
            index += 2
        else:
            index *= 2
